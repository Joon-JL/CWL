#!/usr/bin/env python3
# $Id: hmmer3_phmmer3.py 2106 2017-06-01 17:00:40Z afoix $
# ======================================================================
# HMMER3 phmmer (REST) Python 3 client using urllib3 and
...
#baseUrl = 'http://www.ebi.ac.uk/Tools/services/rest/hmmer3_phmmer'
baseUrl = 'http://ashdev-2.ebi.ac.uk:9889/Tools/services/rest/hmmer3_phmmer'
...
description = """HMMER phmmer is used to search sequences against collections of profiles."""
epilog = """For further information about the HMMER phmmer(REST) web service, see http://www.ebi.ac.uk/Tools/webservices/services/pfa/hmmer_hmmscan_rest"""
version = "$Id: hmmer3_phmmer3.py 2809 2017-06-01 11:52:25Z afoix $"
# Process command-line options
parser = OptionParser(usage=usage, description=description, epilog=epilog, version=version)
# Tool specific options
parser.add_option('--email', help='e-mail address')
parser.add_option('--database', '--sequenceDatabase', help='string  Frequently used[Reference Proteomes:uniprotrefprot, UniProtKB:uniprotkb, SwissProt:swissprot, PDB:pdb]')
parser.add_option('--sequence', help='input sequence file name')

parser.add_option('--alignView', help='Output alignment in result')

parser.add_option('--incE', help='Siginificance E-values[Model] (ex:0.01)')
parser.add_option('--incdomE', help='Siginificance E-values[Hit] (ex:0.03)')
parser.add_option('--E', help='Report E-values[Model] (ex:1)')
parser.add_option('--domE', help='Report E-values[Hit] (ex:1)')

parser.add_option('--incT', help='Significance bit scores[Sequence] (ex:25)')
parser.add_option('--incdomT', help='Significance bit scores[Hit] (ex:22)')
parser.add_option('--T', help='Report bit scores[Sequence] (ex:7)')
parser.add_option('--domT', help='Report bit scores[Hit] (ex:5)')

parser.add_option('--popen', help='Gap Penalties for Open')
parser.add_option('--pextend', help='Gap Penalties for Open')
parser.add_option('--mx', help='Substitution scoring matrix [BLOSUM45, BLOSUM62, BLOSUM90, PAM30, PAM70]')
parser.add_option('--nobias', help='true for turnning off, bias composition filter')
...
# Submit job
    ...
    if options.database:
        params['sequenceDatabase'] = options.database
    elif options.alignView:
        params['alignView'] = options.alignView

    elif options.incE:
        params['incE'] = options.incE
    elif options.incdomE:
